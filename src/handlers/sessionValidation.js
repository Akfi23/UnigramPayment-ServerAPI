...
function startClientCheckIn(request, result) {
    const receivedSecretKey = request.body.secretKey;
    const expectedSecretKey = process.env.CLIENT_SECRET_KEY;

    console.log("Expected Secret Key:", expectedSecretKey);
    console.log("Received Secret Key:", receivedSecretKey);

    if (!receivedSecretKey || receivedSecretKey !== expectedSecretKey) {
        return result.status(401).json({
            error: 'Unauthorized client, access denied.',
            received: receivedSecretKey ? receivedSecretKey : null,
...
function authenticateBot(request, result, next) {
    ...
    const token = authHeader.split(' ')[1];
    const decryptedToken = crypto.AES.decrypt(token, process.env.BOT_SECRET_KEY).toString(crypto.enc.Utf8);
