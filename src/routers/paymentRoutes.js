...
// const { requireEncryptedAuth } = require('../handlers/encryptedAuth'); // <-- новый middleware
...
// router.post('/update-order-receipt',
//     // заменяем authenticateBot на проверку зашифрованного заголовка
//     requireEncryptedAuth, (request, result) => 
// {
//     try {
//         const latestReceipt = request.body;
...
//         console.log(`[${time.getCurrentTimestamp()}] Received payment data: ${JSON.stringify(latestReceipt)}`);

//         if (!latestReceipt.provider_payment_charge_id || !latestReceipt.telegram_payment_charge_id) {
//           console.warn('[ServerAPI] update-order-receipt missing expected fields', JSON.stringify(latestReceipt));
//           return result.status(400).json({ error: 'bad_request', message: 'missing payment identifiers' });
//         }

//         const buyerTelegramId = String(latestReceipt.provider_payment_charge_id).split('_')[0];
...
//     } catch (e) {
//         console.error('[ServerAPI] update-order-receipt handler error:', e && e.stack ? e.stack : e);
//         return result.status(500).json({ error: 'internal_server_error', message: e.message });
//     }
// });
